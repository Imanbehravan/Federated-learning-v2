...
const
MERKLE_TREE_ROOT = 2910591527128539415019962172816527416726075261626525006129358159926565303954;

func
matrix_flattening(m: felt **, step: felt, row: felt, col: felt, cols: felt, res: felt *){
    alloc_locals;
if (step == 0)
{
return ();
...
{hash_ptr: HashBuiltin *}(array: felt *, index: felt, res: felt *){
alloc_locals;
if (index == 0) {
return ();
}
...
let(hash) = hash2
{hash_ptr = hash_ptr}([array], [array + 1]);
assert [res] = hash;
...
func
build_merkle_root
{hash_ptr: HashBuiltin *}(counter: felt, res: felt **) -> (merkle_root : felt)
{
alloc_locals;
...
build_merkle_tree_level
{hash_ptr = hash_ptr}(array =[res - 1], index = counter, res = newRow);
...
alloc_locals;
let
X_without_precision: felt ** = alloc();
let(local
r1: felt *) = alloc();
let(local
r2: felt *) = alloc();
...
index += 1
print("data: ", program_input["X_without_precision"])
print("X data in Cairo: ", memory[ids.X_without_precision])
%}


let(local
Y_without_precision: felt **) = alloc();
let(local
r1: felt *) = alloc();
